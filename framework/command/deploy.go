...
import (
	...
	"github.com/JiadeXu/jade/framework/provider/ssh"
...
func deployBuildBackend(c *cobra.Command, deployFolder string) error {
	...
	if err != nil {
		log.Fatalln("jade go: 请在Path路径中先安装go")
	}
	...
	if configService.GetString("deploy.backend.goos") != "" {
		goos := "GOOS=" + configService.GetString("deploy.backend.goos")
		cmd.Env = append(cmd.Env, goos)
	}
	if configService.GetString("deploy.backend.goarch") != "" {
		goarch := "GOARCH=" + configService.GetString("deploy.backend.goarch")
		cmd.Env = append(cmd.Env, goarch)
	}
...
func deployUploadAction(deployFolder string, container framework.Container, end string) error {
	...
	// 遍历所有deploy的服务
	deployModes := configService.GetStringSlice("deploy.connections")
	if len(deployModes) == 0 {
	...
	for _, node := range deployModes {
		...
		for _, action := range preActions {
			...
			if err != nil {
				session.Close()
				logger.Error(context.Background(), "execute pre action error", map[string]interface{}{
					"cmd":        action,
					"connection": node,
					"err":        err,
				})
		...
		for _, action := range postActions {
			...
			if err != nil {
				session.Close()
				session.Close()
				logger.Error(context.Background(), "execute post action error", map[string]interface{}{
					"cmd":        action,
					"connection": node,
					"err":        err,
				})
