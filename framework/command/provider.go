...
var providerListCommand = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		jadeContainer := container.(*framework.JadeContainer)
		// 获取字符串凭证
		list := jadeContainer.NameList()
...
var providerCreateCommand = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// 检查服务是否存在
		providers := container.(*framework.JadeContainer).NameList()
		...
		funcs := template.FuncMap{"title": strings.Title}
		{
			...
			// 使用contract模板来初始化template，并且让这个模板支持title方法 即支持{{.|title}}
